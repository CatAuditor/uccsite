...
```
build.js                 build: validate → render → dist/ (+ generated sitemap.xml)
package.json             `npm run build` / `npm run dev` (wrangler pages dev)
templates/*.html         one per page (14)
...
Hard rules:
- **Do not add page HTML at repo root.** Every page is a template in `templates/` and an entry in `PAGES` in `build.js`. (`tip.html` and `privacy.html` moved into `templates/` 2026-08-23.)
...
`sitemap.xml` is generated by the build from this table (`sitemap: false` excludes; `lastmod` from file mtimes). Do not check in a sitemap.

## Statements

Official statements live in `content/statements.json` (`statements` array: `slug`, `date`, `topic`, `author`, `title`, `snippet`, `body` markdown, `signoff`), newest first. `build.js` converts `body` via `mdToHtml` and renders one `<article id={{slug}}>` per statement on `/statements.html`. The homepage card is **derived** from the first entry at build time — there is no second copy to maintain.

## Nav / footer

`templates/partials/header.html` and `footer.html`, included with `{{> header}}` / `{{> footer}}` on every page except `success.html`. The header partial opens `<main id="main">` (after a skip link); the footer partial closes it. `aria-current="page"` is set from `current.<page>`; subpages get the `scrolled` header class via `{{^is_home}}`.
