...
Set `author` on the new entry. A missing `author` renders as a bare "By " — the field is required, not optional. Projects, statements, and issues all have an Author field in their Decap collections (`static/admin/config.yml`).
