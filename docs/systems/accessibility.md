...
- **Dialogs are inert when hidden.** The donate modal uses `visibility:hidden` + the `inert` attribute + `aria-hidden` until shown; `aria-modal` is only present while open; focus moves to the close button and is trapped, then restored on close.
