...
| apps/admin/app/media/uploader.js (browser console) | PUT or action failed | `[media] upload failed <Error>` | none | S3 403 = CORS origin not allowed, presign expired, or Content-Type differs from the one presigned (it is signed) |
...
crashed mid-run (check the alarm / CloudWatch); `failed` = see `error`. The page marks pending/processing rows with no update for 15 min as stalled and stops polling.
